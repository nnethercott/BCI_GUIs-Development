...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow):
        ...
        self.m = PlotCanvas(MainWindow, width=690, height=250)
        self.m.move(40,250)
        ...
        self.configure_settings_options.setShortcutVisibleInContextMenu(True)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.configure_settings_options.setShortcut(_translate("MainWindow", "Ctrl+E"))
    ...
    def record(self):
        ...
        if self.timestamps_box.isChecked() is True:
            timestamps = np.linspace(0,int(length), int(length)*sr)

        #dont know how method above does not work :(
        #consider changing num samples in linspace to length of the column eeg_data
        timestamps = timestamps[:len(eeg_data[:,0])]

        #convert data to a dataframe for ease of exporting
        data = pd.DataFrame()
        if self.timestamps_box.isChecked() is True:
        ...
        self.m.plot(eeg_data)



    def save(self, data):
        self.exportable_data.to_csv(self.file_search.text(), index=False)
